...
ServerManager::ServerManager(const std::vector<ServerConfig>& servers) : _running(false), _servers(servers)
{
}
		...
		try
		{
			...
			for (it = host_ports.begin(); it != host_ports.end(); ++it)
			{
				...
				for (size_t j = 0; j < ports.size(); ++j)
				{
					...
					{
						close(fd);
						throw std::runtime_error("Failed to add socket to epoll for ");
					}
...
void ServerManager::checkTimeouts()
{
	...
	for (std::map<int, Client*>::iterator it = _clients.begin(); it != _clients.end(); )
	{
		...
		{
			int fd = it->first;
			LOG_DEBUG("Client timed out fd" + Utils::toString(fd));
