...
void HTTPResponse::buildResponse() 
{
	...
	else if (_request->getLocation().isAutoIndexOn() && Utils::isDirectory(_request->getResource())) 
		handleAutoIndex();
...
{
	std::string page = _request->getServer().getErrorPage(statusCode);
	if (Utils::isFileExists(page) && !Utils::isFileReadble(page)) 
		return(buildErrorResponse(403));

	setBodyResponse(page);
	setProtocol(_request->getProtocol());
...
void HTTPResponse::handleGet() 
{
	...
	else 
	{
		...
		setHeader("Date", Utils::getCurrentDate());
		setHeader("Connection", shouldKeepAlive() ? "keep-alive" : "close");
		setHeader("Content-Type", Utils::getMimeType(resource));
		setBodyResponse(resource);
		setHeader("Content-Length", Utils::toString(getContentLength()));
		buildHeader();
		_isReady = true;
	}
...
{
	int code = _request->getLocation().getRedirectCode();
	std::string target = _request->getLocation().getRedirectPath();
