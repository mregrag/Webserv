...
void ConfigParser::parseServerBlocks(std::string& path) 
{
	std::ifstream file(path.c_str());
	if (!file.is_open())
		throw std::runtime_error("Cannot open file: " + path);
	...
	while (std::getline(file, line)) 
	{
		...
		if (bracecount == 0 && !serverFound && line.find("server") != std::string::npos) 
		{
			serverFound = true;
	...
	while (std::getline(stream, line))
	{
		if (line.find("location") == 0)
		{
			...
			if (locationPath.empty())
				throw std::runtime_error("invalid path in Location");
			if (locationPath == "/")
